...
{
    class PrepareForActivitiesViewerExtendActions :
#if ST_2_1
...
#endif
    {
        ...
        IList<IAction> IExtendActivityEditActions.GetActions(IList<IActivity> activities)
        {
            ...
            if (!MiscPlugin.Plugin.PrepareForActivitiesViewerEditMenu) return null;
            ...
            return new IAction[] {
                new PrepareForActivitiesViewerAction(activities2)
            };
        }
        ...
        IList<IAction> IExtendActivityEditActions.GetActions(IActivity activity)
        {
            ...
            IList<IActivity> activities2 = new List<IActivity>();
            activities2.Add(activity);
            return new IAction[] { new PrepareForActivitiesViewerAction(activities2) };
        }
        #endregion
#else
        ...
        {
            if (!MiscPlugin.Plugin.PrepareForActivitiesViewerEditMenu) return new IAction[0];
            if (location == ExtendViewActions.Location.EditMenu)
            {
                return new IAction[] { new PrepareForActivitiesViewerAction(view) };
            }
