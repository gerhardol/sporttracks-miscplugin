/*
Copyright (C) 2008 Gerhard Olsson 
...
{
    class PowerDistanceDiffToCadenceTrackExtendActions : IExtendActivityEditActions, IExtendRouteEditActions
    {
        ...
        IList<IAction> IExtendActivityEditActions.GetActions(IList<IActivity> activities)
        {
            ...
            {
                if (PowerDistanceDiffToCadenceTrackAction.isEnabled(activity))
                {
            ...
            return new IAction[] {
                new PowerDistanceDiffToCadenceTrackAction(activities2)
            };
        }

        IList<IAction> IExtendActivityEditActions.GetActions(IActivity activity)
        {
            if (!PowerDistanceDiffToCadenceTrackAction.isEnabled(activity)) return null;

            return new IAction[] {
                new PowerDistanceDiffToCadenceTrackAction(activity)
            };
        }
